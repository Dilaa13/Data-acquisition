...
cooling_df = pd.read_csv('D:/projects github/Data-acquisition/thermister02/Data_Analyse/cooling.csv', header=None, names=['Row', 'Temp', 'Volt'])
heating_df = pd.read_csv('D:/projects github/Data-acquisition/thermister02/Data_Analyse/heating_up.csv', header=None, names=['Row', 'Temp', 'Volt'])
...
cooling_fit = Polynomial.fit(cooling_df['Temp'], cooling_df['Volt'], 2)
heating_fit = Polynomial.fit(heating_df['Temp'], heating_df['Volt'], 2)
...
plt.title('Residuals using 2nd Degree Polynomial Fit')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
